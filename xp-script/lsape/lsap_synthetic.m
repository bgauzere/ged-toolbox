...
N=100;
distance_synthetic_lsap = zeros(N,length(sizes));
mapping_times_lsap = zeros(length(sizes),1);
...
for cursize = 1:length(sizes)
    ...
    for i = 1:N
        ...
        mapping_times_lsap(cursize) =  mapping_times_lsap(cursize) + mapping_time;
        distance_synthetic_lsap(i,cursize) = val;
    end
end
