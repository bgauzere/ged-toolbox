function [mapping, running_time] = ipfpe_zeta(G1, G2, ...
    ...
    % n=size(G1,1);
    % m=size(G2,1);
    CostMatrix = NodeCostMatrixLSAPE(G1, G2, costs); %% Pas besoin de
                                                %% recalculer !
    Xk = Mapping_init; 
    [phi_i, i] = find(Xk');
    ...
    S_zeta(1) = (1-nu)*S(1) + zeta*linearMul(Xk,Xk); % Premier
                                                     % terme
                                                     % inutile pour
                                                     % zeta = 1
    ...
    while ((k < maxIter)  && (norm(Xk - Xkm1,1) > 0.0001));
        ...
        [phi_i, i] = find(bkp1');
        Xkp1tD = quadraticTermLSAPE(G1, G2, costs.cei, costs.ced, costs.ces, bkp1,[i, phi_i]);
