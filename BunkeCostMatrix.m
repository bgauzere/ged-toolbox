...
for i=1:n
    ...
    if(numel(a1) == 0)
        ...
        for j=1:m
            if(sub_m{j} ~= 0)
                CM(i,j) = numel(edges2)*costs.ced + numel(nodes2)*costs.cnd ...
                          + costs.cns*(G1(i,i) ~= G2(j,j));
            else
                CM(i,j) = costs.cns*(G1(i,i) ~= G2(j,j));
            end
        ...
        for j=1:m
            ...
            sub_cost = [sub_costA,sub_costR; sub_costI, ...
                        zeros(sub_m{j},sub_n)];
                        [mapping,~,~] = hungarianLSAP(sub_cost);
            CM(i,j) = sum(sum(sub_cost((mapping'*(sub_n+sub_m{j}))+int32([1:length(mapping)])))) + ...
                      costs.cns*(G1(i,i) ~= G2(j,j));
            else
                CM(i,j) = numel(edges1)*costs.cei + numel(nodes1)*costs.cni  +costs.cns*(G1(i,i) ~= G2(j,j));
            end
            CM(n+j,j) = length(a2{j})*(costs.cni + costs.cei) + costs.cni;
            CM(i,m+i) = length(a1)*(costs.cnd + costs.ced) + costs.cnd;
        end
